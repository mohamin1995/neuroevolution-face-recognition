...
BPNN *bpnn_create(n_in, n_hidden, n_out,weight)
int n_in, n_hidden, n_out;
double* weight;
{
  ...
  newnet = bpnn_internal_create(n_in, n_hidden, n_out);
	...
	int i,j;
    int inputweight_n = ((n_in+1) * (n_hidden+1));
	int hiddenweight_n = ((n_hidden+1) * (n_out+1));
	double *inputweight = alloc_1d_dbl(inputweight_n+1);
	double *hiddenweight = alloc_1d_dbl(hiddenweight_n+1);

	for (i = 0; i < inputweight_n; i++) {
		inputweight[i] = weight[i];
	}
	
	i++;

	for (j=0; j<hiddenweight_n; j++) {
		hiddenweight[j] = weight[i];
		i++;
	}
	
	set_weights(newnet->input_weights, n_in, n_hidden, inputweight);

	set_weights(newnet->hidden_weights, n_hidden, n_out, hiddenweight);

	free((double *)inputweight);
	free((double *)hiddenweight);
